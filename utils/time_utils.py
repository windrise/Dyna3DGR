...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, t_multires=6, multires=10,
                 local_frame=False, max_d_scale=-1, **kwargs):
    ...
    def trainable_parameters(self):
        return [
            {'params': self.gaussian_warp.parameters(), 'name': 'warp'},
            {'params': self.gaussian_scaling.parameters(), 'name': 'scaling'},
            {'params': self.gaussian_rotation.parameters(), 'name': 'rotation'},
            {'params': self.linear.parameters(), 'name': 'backbone'},
        ]
    ...
    def forward(self, x, t, **kwargs):
        ...
        scaling = 2*torch.sigmoid(self.gaussian_scaling(h))
...
class ControlNodeWarp(nn.Module):
    def __init__(self, init_pcl=None, node_num=512, K=3, enable_densify_prune=False, with_arap_loss=False, with_node_weight=True, local_frame=False, d_rot_as_res=True, skinning=False, hyper_dim=2, max_d_scale=-1, is_scene_static=False, opt = None, **kwargs):
        super().__init__()
        self.K = K
        ...
        if self.is_scene_static:
            ...
            self.network = DeformNetwork(local_frame=local_frame, max_d_scale=max_d_scale).cuda()
        ...
        if init_pcl is not None:
            self.init(opt = opt, init_pcl=init_pcl)
    ...
    def init(self, opt, init_pcl, hyper_pcl=None, keep_all=False, force_init=False, as_gs_force_with_motion_mask=False, force_gs_keep_all=False, reset_bbox=True, **kwargs):
        ...
        if force_gs_keep_all:
            self.init_gaussians_ca(init_pcl=init_pcl, with_motion_mask=as_gs_force_with_motion_mask)
        else:
            self.init_gaussians_ca(init_pcl=self.nodes[..., :3],with_motion_mask=as_gs_force_with_motion_mask)
    ...
    @property
    def as_gaussians_ca(self):
        if not hasattr(self, 'gs') or self.gs is None:
            # Building Learnable Gaussians for Nodes
            # print('Building Learnable Gaussians for CA Nodes!')
            from scene.gaussian_model import GaussianModel, BasicPointCloud, StandardGaussianModel
            pcd = BasicPointCloud(points=self.nodes[..., :3].detach(), colors=torch.zeros_like(self.nodes[..., :3]), normals=self.nodes[..., :3].detach())
            self.gs = StandardGaussianModel( all_the_same=True, with_motion_mask=False)  # blender datas are all dynamic
            self.gs.create_from_points_cloud(pcd=pcd, spatial_lr_scale=0., print_info=True)
            self.gs._scaling.data = torch.log(1e-2 * torch.ones_like(self.gs._scaling))
            self.gs._xyz.data = self.nodes[..., :3]
        return self.gs
    def init_gaussians(self, init_pcl, with_motion_mask):
        if not hasattr(self, 'gs') or self.gs is None:
            ...
            self.gs.create_from_pcd(pcd=pcd, spatial_lr_scale=0., print_info=False)
        return self.gs
    def init_gaussians_ca(self, init_pcl, with_motion_mask):
        if not hasattr(self, 'gs') or self.gs is None:
            # Building Learnable Gaussians for Nodes
            print('Initialize Learnable Gaussians for Nodes with Point Clouds!')
            from scene.gaussian_model import GaussianModel, BasicPointCloud, StandardGaussianModel
            pcd = BasicPointCloud(points=init_pcl.detach(), colors=torch.zeros_like(init_pcl), normals=torch.zeros_like(init_pcl))
            self.gs = StandardGaussianModel( all_the_same=True, with_motion_mask=with_motion_mask)  # blender datas are all dynamic
            self.gs.create_from_points_cloud(pcd=pcd, spatial_lr_scale=0., print_info=True)
        return self.gs
